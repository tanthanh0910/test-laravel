...
$('#toggleSideBar').click(function () {
  $('#mySidebar').toggleClass('open-side-bar')
  $('#main').toggleClass('open-side-bar')
})

$('nav ul li.has-submenu').click(function () {
  $(this).toggleClass('opened')
})

window.addEventListener('resize', function (event) {
  var newWidth = window.innerWidth
  const elementSidebar = document.getElementById('mySidebar')
  const elementMain = document.getElementById('main')
  if (newWidth > 1200) {
    if (elementSidebar && !elementSidebar.classList.contains('open-side-bar')) {
      elementSidebar.classList.add('open-side-bar')
    }
    if (elementMain && !elementMain.classList.contains('open-side-bar')) {
      elementMain.classList.add('open-side-bar')
    }
  } else {
    if (elementSidebar && elementSidebar.classList.contains('open-side-bar')) {
      elementSidebar.classList.remove('open-side-bar')
    }

    if (elementMain && elementMain.classList.contains('open-side-bar')) {
      elementMain.classList.remove('open-side-bar')
    }
  }
  
})
$('.custom-select').change(function () {
  $(this).css('color', $(this).val() === '' ? '#999' : '#000');
});
